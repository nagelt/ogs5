...
double SolidMinkley::A(const double theta, const double alpha)
{
	...
	A = std::cos(thetaT) / 3.
	    * (3. + std::tan(thetaT) * std::tan(3. * thetaT)
	       + 1. / std::sqrt(3.) * sgn(theta) * (std::tan(3. * thetaT) - 3. * std::tan(thetaT)) * std::sin(alpha));
	return A;
}
...
double SolidMinkley::B(const double theta, const double alpha)
{
	...
	B = 1. / (3. * std::cos(3. * thetaT))
	    * (sgn(theta) * std::sin(thetaT) + std::sin(alpha) * std::cos(thetaT) / std::sqrt(3.));
	return B;
}
	...
	if (std::abs(theta) < thetaT)
	{
		F = SolidMath::CalI1(sig) / 3. * std::sin(phi)
		    + std::sqrt(SolidMath::CalJ2(sigd))
		          * (std::cos(theta) - 1. / std::sqrt(3.) * std::sin(phi) * std::sin(theta))
		    - coh * std::cos(phi);
	}
	else
		F = SolidMath::CalI1(sig) / 3. * std::sin(phi)
...
/**************************************************************************
   ...
   06/2015 TN Implementation
**************************************************************************/
void SolidMinkley::CalViscoelasticResidual(const double dt, const KVec& dstrain_curr, const double e_curr,
                                           const double e_p_curr, const KVec& stress_curr, const KVec& dstrain_Kel_curr,
                                           const KVec& dstrain_Kel_t, const KVec& dstrain_Max_curr,
                                           const KVec& dstrain_Max_t, const KVec& dstrain_p_curr,
...
/**************************************************************************
   ...
   06/2015 TN Implementation
**************************************************************************/
void SolidMinkley::CalViscoelasticJacobian(const double dt, const KVec& stress_curr, const double sig_eff,
                                           Eigen::Matrix<double, 18, 18>& Jac)
{
	...
	// build G_31
	Jac.block<6, 6>(12, 0) = -1. / (2. * etaM) * (GM * SolidMath::P_dev - sigd_curr * dmu_vM.transpose() / etaM);
...
void SolidMinkley::CaldGdE(Eigen::Matrix<double, 18, 6>& dGdE)
{
	...
	dGdE.setZero(18, 6);
	dGdE.block<6, 6>(0, 0) = -2. * SolidMath::P_dev - 3. * KM / GM * SolidMath::P_sph;
}
...
void SolidMinkley::CalViscoplasticResidual(const double dt, const KVec& dstrain_curr, const double e_curr,
                                           ...
                                           const double e_pl_vol_t, const double e_pl_eff_curr, const double e_pl_eff_t,
                                           const double lam_curr, Eigen::Matrix<double, 27, 1>& res)
{
	const KVec sigd_curr(GM * SolidMath::P_dev * stress_curr);
	const double J_2(SolidMath::CalJ2(sigd_curr)), J_3(SolidMath::CalJ3(sigd_curr)),
	...
	res.block<1, 1>(25, 0)(0) = (e_pl_eff_curr - e_pl_eff_t) / dt
	...
	else
	{
		...
		Ddev_flowDsigma
		    = ((DG_DJ2(theta, J_2, psi) + DGDtheta * DthetaDJ2) * SolidMath::P_dev
		       ...
		       + DGDtheta * J_3 * (DthetaDJ3 + DDtheta_DDJ3(theta, J_3) * J_3) * dev_sigd_curr_inv
		       ...
		       + J_3 * (DDG_DDtheta(theta, J_2, psi) * DthetaDJ3 + DGDtheta * DDtheta_DJ3_Dtheta(theta, J_3))
		             * (DthetaDJ2 * dev_sigd_curr_inv * sigd_curr.transpose()
		                + DthetaDJ3 * J_3 * dev_sigd_curr_inv * dev_sigd_curr_inv.transpose())) * GM;
	}
...
/**************************************************************************
   ...
   Task: Calculates the 27x6 derivative of the residuals with respect to total strain. Implementation fully implicit
only.
   Programing:
   06/2015 TN Implementation
**************************************************************************/
