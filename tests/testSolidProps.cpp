...
TEST(SolidProps, YieldMohrCoulomb)
{
	...
	// MC formulation
	sig_i(0) = -2.; // sigma_1
	sig_i(2) = sig_i(0) * (1. + std::sin(phi)) / (1. - std::sin(phi))
	           - 2. * (*data)(7) * std::cos(phi) / (1. - std::sin(phi)); // sigma_3
		...
		for (size_t j = 0; j < 18; j++)
		{
			...
			else if (j < 12)
			{
				...
				material.CalViscoelasticResidual(dt, epsd_i, e_i, e_pv_j, sig_j, upper, eps_K_t, eps_M_j, eps_M_t,
				                                 eps_pl_j, residual);
				up = residual(i);
				material.CalViscoelasticResidual(dt, epsd_i, e_i, e_pv_j, sig_j, lower, eps_K_t, eps_M_j, eps_M_t,
				                                 eps_pl_j, residual);
				low = residual(i);
			}
			else
			{
				...
				material.CalViscoelasticResidual(dt, epsd_i, e_i, e_pv_j, sig_j, eps_K_j, eps_K_t, upper, eps_M_t,
				                                 eps_pl_j, residual);
				up = residual(i);
				material.CalViscoelasticResidual(dt, epsd_i, e_i, e_pv_j, sig_j, eps_K_j, eps_K_t, lower, eps_M_t,
				                                 eps_pl_j, residual);
				low = residual(i);
			}
			Jac_num(i, j) = (up - low) / (2. * pertub);
...
TEST(SolidProps, TensorInversion)
{
	...
	vec(1) = -1.3;
	vec(2) = 0.8;
	vec(3) = 1.1 * std::sqrt(2.);
	vec(4) = -2.0 * std::sqrt(2.);
	vec(5) = 0.3 * std::sqrt(2.);
		...
		for (size_t j = 0; j < 27; j++)
		{
			...
			else if (j < 12)
			{
				...
				material.CalViscoplasticResidual(dt, epsd_i, e_i, sig_j, upper, eps_K_t, eps_M_j, eps_M_t, eps_pl_j,
				                                 eps_pl_t, e_v_i, 0., e_eff_i, 0., lam_j, residual);
				up = residual(i);
				material.CalViscoplasticResidual(dt, epsd_i, e_i, sig_j, lower, eps_K_t, eps_M_j, eps_M_t, eps_pl_j,
				                                 eps_pl_t, e_v_i, 0., e_eff_i, 0., lam_j, residual);
				low = residual(i);
			}
			else if (j < 18)
			{
				...
				material.CalViscoplasticResidual(dt, epsd_i, e_i, sig_j, eps_K_j, eps_K_t, upper, eps_M_t, eps_pl_j,
				                                 eps_pl_t, e_v_i, 0., e_eff_i, 0., lam_j, residual);
				up = residual(i);
				material.CalViscoplasticResidual(dt, epsd_i, e_i, sig_j, eps_K_j, eps_K_t, lower, eps_M_t, eps_pl_j,
				                                 eps_pl_t, e_v_i, 0., e_eff_i, 0., lam_j, residual);
				low = residual(i);
			}
			else if (j < 24)
			{
				...
				material.CalViscoplasticResidual(dt, epsd_i, e_i, sig_j, eps_K_j, eps_K_t, eps_M_j, eps_M_t, upper,
				                                 eps_pl_t, e_v_i, 0., e_eff_i, 0., lam_j, residual);
				up = residual(i);
				material.CalViscoplasticResidual(dt, epsd_i, e_i, sig_j, eps_K_j, eps_K_t, eps_M_j, eps_M_t, lower,
				                                 eps_pl_t, e_v_i, 0., e_eff_i, 0., lam_j, residual);
				low = residual(i);
			}
			else if (j < 25)
